package com.sports.service.student;
...
import com.sports.controller.student.ClassTeacherController;
import com.sports.service.athlete.NumberRuleService;

/**
 * 班主任端业务逻辑（M1 修复：从 ClassTeacherController 抽取，避免 Controller 越层 + @Transactional 标在 Controller 上）。
 *
 * <p>班主任只能操作自己绑定的班级。本 Service 承载名单导入 / 运动员管理 / 报名 / 赛程 / 成绩等业务，
 * 事务边界统一收敛在 Service 层。</p>
 */
@Slf4j
@Service
...
public class ClassTeacherService {
    ...
    @Transactional
    public ApiResponse<?> importRoster(MultipartFile file, Long classId) {
    ...
    @Transactional
    public ApiResponse<?> addAthlete(Map<String, Object> body) {
    ...
    public ApiResponse<?> athletes(int page, int size) {
        Long classId = getMyClassId();
    ...
    // ===== 为运动员报名项目 =====
    @Transactional
    public ApiResponse<?> registerAthlete(Map<String, Object> body) {
        ...
        if (!GenderUtil.matches(event.getGenderLimit(), athlete.getGender())) {
    ...
    // ===== 取消报名 =====
    @Transactional
    public ApiResponse<?> cancelRegistration(Long id) {
