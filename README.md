...
基于 **Spring Boot 3.4 + Vue 3 + Element Plus** 的全栈运动会管理系统。支持**管理员 / 体育老师 / 班主任 / 学生**多角色协作，覆盖**班级名单导入 → 运动会报名 → 智能编排 → 项目编排 → 成绩录入 → 排名积分 → 报表导出**全流程。
...
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘   │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐   │
│  │ 编排算法 │ │ 项目编排 │ │ 排名积分 │ │ Excel    │   │
│  │ 贪心+优化│ │ 赛程调度 │ │ 奖牌汇总 │ │ EasyExcel│   │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘   │
└──────────────────────────┬───────────────────────────────┘
                           │
┌──────────────────────────▼───────────────────────────────┐
...
#### 🎛️ 规则完全自定义（系统设置 → 号码簿规则 / 编排规则 / 积分规则）

| 规则 | 配置项 | 说明 |
|------|--------|------|
| 号码簿规则 | `{grade}{class}{seq:02d}` 等模板变量 + 年级映射 + 自动补零/提取班级序号 | 号码生成模板完全自定义，支持 `{grade}` `{class}` `{seq:NNd}` `{gender}` `{year}` `{school_code}` 等变量与实时预览 |
| 编排规则 | 软约束（同班不同组/道、禁止同班同组、同年级打散、成绩优秀者居中、同班同组人数上限）+ 算法参数（最大尝试次数/超时/优化轮数） | 编排算法按配置执行，道次编排支持中道优先分配 |
| 积分规则 | 名次积分表 + 并列处理 + 破纪录加分 + 参与分 + 接力倍数 + 团体总分口径/排序 | 排名积分完全自定义 |
