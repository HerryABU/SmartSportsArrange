...
> Sports Meet Intelligent Arrangement System v2.6.8
...
# 自定义端口 + 绑定地址（推荐写法）
java -jar sports-2.6.8.jar --app.port=8899 --app.host=::

# 等价的 Spring 标准写法
java -jar sports-2.6.8.jar --server.port=9090

# 后台运行
nohup java -jar sports-2.6.8.jar --app.port=8899 > app.log 2>&1 &
```
...
| 方式 | 操作 | 生效方式 |
|------|------|----------|
| ① 命令行参数 | `java -jar sports-2.6.8.jar --app.port=8899 --app.host=::`<br>`.\start.ps1 -Port 8899 -Host ::` / `start.bat --app.port=8899`<br>（也可用标准 `--server.port=9090`） | 立即（本次运行） |
| ② 环境变量 | `SERVER_PORT=9090 java -jar sports-2.6.8.jar`（Linux/macOS）<br>`$env:SERVER_PORT="9090"; java -jar ...`（PowerShell） | 立即（本次运行） |
...
# 输出
copy sports-backend\target\sports-2.6.8.jar .
```
